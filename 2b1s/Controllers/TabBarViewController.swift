...
extension TabBarViewController {
  fileprivate func loadItinerary() {
    ...
    _ = Event(context: store.context, name: "Dinner", photoName: photo6, time: 1496626200, rating: 0, order: 5)
    _ = Event(context: store.context, name: "Cake", photoName: photo7, time: 1496631600, rating: 0, order: 6)
    _ = Event(context: store.context, name: "Present", photoName: photo8, time: 1496635200, rating: 0, order: 7)
  ...
  func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
    ...
    case .update:
      ...
      if let homeVC = self.viewControllers?.first as? HomeViewController {
        if homeVC.isViewLoaded && self.selectedIndex != 0 {
